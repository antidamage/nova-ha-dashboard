"""MiniFASNet — vendored, unmodified, from Silent-Face-Anti-Spoofing.

Provenance
    Upstream: https://github.com/minivision-ai/Silent-Face-Anti-Spoofing
    File:     src/model_lib/MiniFASNet.py
    Author:   zhuying, Minivision
    Licence:  Apache License 2.0 (see upstream LICENSE)

Vendored rather than installed because upstream publishes no package, and the
`.pth` weights the Dockerfile fetches only load against this exact module — the
`keep_dict` channel lists and the attribute names are the state-dict keys. A
reimplementation from memory would silently fail to load, and a service that
cannot detect a photograph must not serve, so this is a verbatim copy.

Weights are not committed. The Dockerfile fetches them by pinned URL and
verifies them with `sha256sum -c`.

Everything below this header is upstream's code as published.
"""
