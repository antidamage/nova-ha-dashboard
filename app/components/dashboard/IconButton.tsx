...
export function IconButton({
  ...
  const demoTooltipTitle = label.startsWith("On: ") ? "On" : label;
  const demoTooltipText = label === "Off" ? "Turn selected lights off." : "Apply this light preset.";
  const button = (
    <MomentaryFeedbackButton
      type="button"
      aria-label={label}
      data-demo-tooltip-title={demoTooltipTitle}
      data-demo-tooltip={demoTooltipText}
      onClick={onClick}
      disabled={disabled}
