...
export function RouterPanel({
  ...
  // Two sub-panels, neither with an Advanced section (specs/advanced-fold.md):
  // everything for the WAN and the router, and the computers beside it.
  return (
    <div className="router-panel-grid grid gap-5">
      <AdvancedFold className="router-panel border border-neutral-700 bg-neutral-950/70 p-5">
      ...
      {onDesktopSleep || onDesktopWake ? (
        <AdvancedFold className="router-computers border border-neutral-700 bg-neutral-950/70 p-5">
          <DesktopSleepPanel
            sleepBusy={desktopSleepBusy}
            wakeBusy={desktopWakeBusy}
            onSleep={onDesktopSleep}
            onWake={onDesktopWake}
          />
        </AdvancedFold>
      ) : null}
    </div>
