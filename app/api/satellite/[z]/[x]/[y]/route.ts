...
function validTile(z: number, x: number, y: number, maxZoom = MAX_ZOOM) {
  if (!Number.isInteger(z) || !Number.isInteger(x) || !Number.isInteger(y) || z < 0 || z > maxZoom) {
    return false;
  }
  const tileCount = 2 ** z;
...
function satelliteUpstreamUrl(z: number, x: number, y: number, template = SATELLITE_TILE_URL) {
...
export async function GET(request: Request, { params }: { params: Promise<SatelliteTileParams> }) {
  ...
  if (!validTile(z, x, y, config.mapWeather.satellite.maxZoom)) {
  ...
  try {
    const upstream = await fetch(satelliteUpstreamUrl(z, x, y, config.mapWeather.satellite.tileUrlTemplate), {
      cache: "no-store",
