...
test.describe("configuration workspace", () => {
  test("renders the workspace sections without errors", async ({ page }) => {
    ...
    for (const section of ["Config Import/Export", "Camera", "Secrets", "Updates", "System Power"]) {
      await expect(page.getByRole("button", { name: new RegExp(section.replace("/", "\\/")) })).toBeVisible();
    }
