// The demo-mode `fetch`/`EventSource` shim, injected into the static-export
// page as an inline `<script>`. It is one continuous piece of browser JS
// assembled from one template literal — section 2 criterion 3 of
// specs/agent-token-footprint.md: splitting it would force several
// string-fragment exports whose only purpose is being concatenated back
// together by demoConfigBootstrapScript itself, which is a worse seam than
// the size cap it would satisfy.
...
import { demoDashboardConfig } from "./fixtures-model";
import type { DemoThemeEnvelope, DemoThemeLibrary } from "./types";
