...
export function parseTaskCsv(text: string, referenceDate: Date): ParseTaskCsvResult {
  ...
  text.split(/\r?\n/).forEach((rawLine, index) => {
    ...
    if (!fields.name) {
      errors.push({ line: lineNumber, message: "Reminder name is required" });
      return;
    }
    ...
    // `parsedRepeat` maps both the explicit no-repeat keywords and unrecognised
    // text to undefined; only the latter is an error. Without this distinction
    // the documented "none"/"no repeat" values would be rejected.
    const repeatText = fields.repeat.trim().toLowerCase();
    const explicitNoRepeat = repeatText === "none" || repeatText === "no repeat";
    const repeat = parsedRepeat(fields.repeat);
    if (fields.repeat && !repeat && !explicitNoRepeat) {
